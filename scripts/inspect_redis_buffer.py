...
def inspect_buffer(client: redis.Redis, key: str, show_details: bool = True):
    ...
    if show_details:
        ...
        print(
            f"[{ts_now}] Count: {count} | Oldest: {format_timestamp(oldest_ts)} | Newest: {format_timestamp(newest_ts)} | Span: {span:.2f}s"
        )


def main():
    parser = argparse.ArgumentParser(
        description="Inspect the real-time Redis flow buffer."
    )
    parser.add_argument(
        "--watch", action="store_true", help="Refresh stats every 5 seconds"
    )
    ...
    try:
        ...
        print(
            f"[!] Could not connect to Redis at {config.REDIS_HOST}:{config.REDIS_PORT}. Error: {e}"
        )
        sys.exit(1)
